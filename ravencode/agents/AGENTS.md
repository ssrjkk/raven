...
4. ПРОВЕРКА КАЧЕСТВА: После создания или изменения кода, используй инструмент `verify` для быстрой проверки синтаксиса (Python/TypeScript/JSON) — а для более глубоких проверок используй `bash` (например, `npm run build`, `python -m py_compile`, `ruff check`), чтобы убедиться, что код работает, прежде чем сообщать об успехе.
