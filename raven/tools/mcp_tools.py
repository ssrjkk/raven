...
def _make_mcp_handler(pool_name: str, tool_name: str, mcp_pool: MCPClientPool | None) -> Any:
...
def register_mcp_tools(registry: ToolRegistry, mcp_pool: MCPClientPool | None) -> None:
    if mcp_pool is None:
        return
...
def create_mcp_plugin_tools(mcp_pool: MCPClientPool | None) -> list[PluginTool]:
