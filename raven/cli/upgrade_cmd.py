...
def upgrade():
    ...
    if (repo / ".git").is_dir():
        ...
        r = subprocess.run(["git", "pull", "--rebase"], capture_output=True, text=True, timeout=60, check=False)
        if r.returncode != 0:
            console.print(f"[red]Git pull failed: {r.stderr.strip()}[/red]")
            raise SystemExit(1)
        console.print("[green]  OK[/green]")
    console.print("[*] Updating Python dependencies...")
    r = subprocess.run([sys.executable, "-m", "pip", "install", "-e", "."], capture_output=True, text=True, timeout=120, check=False)
    if r.returncode != 0:
    ...
    if web_dir.is_dir() and (web_dir / "package.json").is_file():
        ...
        r = subprocess.run(["npm", "install"], capture_output=True, text=True, cwd=str(web_dir), timeout=120, check=False)
        if r.returncode == 0:
            console.print("[green]  OK[/green]")
