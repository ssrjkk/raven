...
class PIIEngine:
    ...
    def _build_patterns() -> list[PIIPattern]:
        return [
            # ── Credentials & Keys ──
            PIIPattern("ssh_key", re.compile(
                r"-----BEGIN\s*(?:RSA|DSA|EC|OPENSSH|PGP)?\s*PRIVATE\s*KEY.*?-----END\s*(?:RSA|DSA|EC|OPENSSH|PGP)?\s*PRIVATE\s*KEY-----",
                re.DOTALL,
            ), "[SSH_KEY]", score=1.0, category="credential"),
            PIIPattern("aws_key", re.compile(
                r"(?:AKIA|ASIA|ABIA|ACCA)[0-9A-Z]{16}",
            ), "[AWS_KEY]", score=0.95, context_keywords=["aws", "amazon", "s3", "ec2"], category="credential"),
            PIIPattern("github_token", re.compile(
                r"(?:ghp_|gho_|ghu_|ghs_|ghr_)[a-zA-Z0-9]{36}",
            ), "[GITHUB_TOKEN]", score=0.95, context_keywords=["github", "git"], category="credential"),
            PIIPattern("slack_token", re.compile(
                r"x(?:ox[abp]|app|opsb|orb)[-0-9a-zA-Z]{24,72}",
            ), "[SLACK_TOKEN]", score=0.95, context_keywords=["slack"], category="credential"),
            PIIPattern("telegram_token", re.compile(
                r"\b\d{8,10}:[a-zA-Z0-9_-]{35}\b",
            ), "[TELEGRAM_TOKEN]", score=0.95, context_keywords=["telegram", "bot"], category="credential"),
            PIIPattern("discord_token", re.compile(
                r"[a-zA-Z0-9_-]{24}\.[a-zA-Z0-9_-]{6}\.[a-zA-Z0-9_-]{27}",
            ), "[DISCORD_TOKEN]", score=0.9, context_keywords=["discord"], category="credential"),
            PIIPattern("jwt", re.compile(
                r"eyJ[a-zA-Z0-9_-]{4,}\.[a-zA-Z0-9_-]{4,}\.[a-zA-Z0-9_-]{2,}",
            ), "[JWT]", score=0.9, context_keywords=["jwt", "token", "bearer"], category="credential"),
            PIIPattern("api_key_generic", re.compile(
                r"(?:(?:sk|pk|api|key|secret|token)[-_])[a-zA-Z0-9]{16,64}",
            ), "[API_KEY]", score=0.7, context_keywords=["key", "secret", "token", "api", "password", "auth"],
             category="credential"),
            PIIPattern("connection_string", re.compile(
                r"\b(?:postgres|mysql|mongodb|redis|amqp|rabbitmq)://[^\s<>\"']+",
            ), "[CONNECTION_STRING]", score=0.95, category="credential"),
            PIIPattern("basic_auth_url", re.compile(
                r"\bhttps?://[^\s:<>\"']+:[^\s@<>\"']+@[^\s<>\"']+",
            ), "[URL_WITH_CREDENTIALS]", score=0.95, category="credential"),
            PIIPattern("password_in_text", re.compile(
                r"(?i)(?:password|passwd|pwd|secret)\s*[:=]\s*['\"]?[^\s,;\"']+['\"]?",
            ), "[PASSWORD]", score=0.85, category="credential"),

            # ── Personal Information ──
            PIIPattern("email", re.compile(
                r"[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?"
                r"(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*\.[a-zA-Z]{2,}",
            ), "[EMAIL]", score=0.95, category="personal"),
            PIIPattern("phone_international", re.compile(
                r"\+\d{1,3}[-\s.]?\(?\d{1,4}\)?[-\s.]?\d{1,4}[-\s.]?\d{1,9}",
            ), "[PHONE]", score=0.9, context_keywords=["phone", "call", "tel", "mobile", "cell", "whatsapp"],
             category="personal"),
            PIIPattern("phone_us", re.compile(
                r"\(?\d{3}\)?[-\s.]?\d{3}[-\s.]?\d{4}",
            ), "[PHONE]", score=0.7, category="personal"),
            PIIPattern("ssn", re.compile(
                r"\b\d{3}-\d{2}-\d{4}\b",
            ), "[SSN]", score=0.95, context_keywords=["ssn", "social", "security"], category="personal"),
            PIIPattern("passport_us", re.compile(
                r"\b\d{9}\b",
            ), "[PASSPORT]", score=0.4, context_keywords=["passport", "travel", "visa"], category="personal"),
            PIIPattern("drivers_license", re.compile(
                r"\b[A-Z]\d{3,8}\b",
            ), "[DL]", score=0.3, context_keywords=["license", "dl", "driver", "licence"], category="personal"),
            PIIPattern("dob", re.compile(
                r"\b\d{1,2}[/-]\d{1,2}[/-](?:\d{4}|\d{2})\b",
            ), "[DOB]", score=0.5, context_keywords=["dob", "birth", "born", "birthday", "date of birth"],
             category="personal"),

            # ── Financial ──
            PIIPattern("credit_card_visa_mc", re.compile(
                r"\b(?:\d{4}[-\s]?){3}\d{4}\b",
            ), "[CREDIT_CARD]", score=0.95, context_keywords=["card", "credit", "debit", "payment", "visa",
             "mastercard", "amex"], category="financial"),
            PIIPattern("credit_card_amex", re.compile(
                r"\b3[47]\d{2}[-\s]?\d{6}[-\s]?\d{5}\b",
            ), "[CREDIT_CARD]", score=0.95, category="financial"),
            PIIPattern("iban", re.compile(
                r"\b[A-Z]{2}\d{2}[-\s]?\d{4}[-\s]?\d{4}[-\s]?\d{4}[-\s]?\d{4}[-\s]?\d{0,4}\b",
            ), "[IBAN]", score=0.85, context_keywords=["iban", "bank", "account"], category="financial"),
            PIIPattern("routing_number", re.compile(
                r"\b\d{9}\b",
            ), "[ROUTING_NUMBER]", score=0.3, context_keywords=["routing", "aba", "ach", "bank"], category="financial"),

            # ── Digital Footprints ──
            PIIPattern("ipv4", re.compile(
                r"\b(?:(?:25[0-5]|2[0-4]\d|[01]?\d\d?)\.){3}(?:25[0-5]|2[0-4]\d|[01]?\d\d?)\b",
            ), "[IP_ADDRESS]", score=0.8, category="digital"),
            PIIPattern("ipv6", re.compile(
                r"\b(?:[0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}\b",
            ), "[IP_ADDRESS]", score=0.85, category="digital"),
            PIIPattern("mac_address", re.compile(
                r"\b(?:[0-9A-Fa-f]{2}[:-]){5}[0-9A-Fa-f]{2}\b",
            ), "[MAC_ADDRESS]", score=0.85, category="digital"),
            PIIPattern("bitcoin", re.compile(
                r"\b[13][a-km-zA-HJ-NP-Z1-9]{25,34}\b",
            ), "[BITCOIN_ADDRESS]", score=0.9, category="financial"),
            PIIPattern("ethereum", re.compile(
                r"\b0x[a-fA-F0-9]{40}\b",
            ), "[ETHEREUM_ADDRESS]", score=0.9, category="financial"),
            PIIPattern("hex_token", re.compile(
                r"\b[a-fA-F0-9]{32,64}\b",
            ), "[TOKEN]", score=0.3, context_keywords=["token", "hash", "hex", "session", "csrf", "nonce"],
             category="digital"),
            PIIPattern("session_cookie", re.compile(
                r"(?i)(?:session|sid|token|auth)\s*[:=]\s*['\"]?[a-zA-Z0-9+/]{16,128}={0,2}['\"]?",
            ), "[SESSION_TOKEN]", score=0.75, category="digital"),
        ]

    @staticmethod
    def _build_context_re() -> re.Pattern:
        keywords = set()
...
__all__ = [
    "ContextVisibility", "PIIDetection", "PIIEngine",
    "redact_pii", "mask_pii", "analyze_pii",
    "sanitize_external_content", "filter_context_by_visibility",
]
