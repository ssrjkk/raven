...
class TestImageGenerate:
    def test_dispatch_dalle(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_dispatch_sd(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert asyncio.run(media.image_generate("x", model="sd")) == "sd-ok"

    def test_unknown_model(self) -> None:
        out = asyncio.run(media.image_generate("x", model="foo"))
        assert "Unknown model 'foo'" in out


class TestImageGenerateDalle:
    ...
    def test_no_urls(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(media, "_OPENAI_API_KEY", "sk-test")
    ...
    def test_error(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(media, "_OPENAI_API_KEY", "sk-test")
        _patch_httpx(monkeypatch, _FakeClient(_FakeResp({"data": []}, error=httpx.ConnectError("boom"))))
        out = asyncio.run(media._image_generate_dalle("x", "1024x1024", "standard", 1))
        assert "DALL-E generation failed" in out


class TestImageGenerateSd:
    ...
    def test_default_size(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(media, "_REPLICATE_API_TOKEN", "rpt")
        monkeypatch.setattr(asyncio, "sleep", _noop_sleep)
        post = _FakeResp({"urls": {"get": "https://x/poll"}})
        done = _FakeResp({"status": "succeeded", "output": ["https://x/out.png"]})
        ...
        assert client.last_post_kwargs["json"]["input"]["width"] == 1024

    def test_bad_size_ignored(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(media, "_REPLICATE_API_TOKEN", "rpt")
        monkeypatch.setattr(asyncio, "sleep", _noop_sleep)
        post = _FakeResp({"urls": {"get": "https://x/poll"}})
        done = _FakeResp({"status": "succeeded", "output": ["https://x/out.png"]})
    ...
    def test_httpx_error(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(media, "_REPLICATE_API_TOKEN", "rpt")
        monkeypatch.setattr(asyncio, "sleep", _noop_sleep)
...
class TestImageEdit:
    def test_success(self, ws: Path) -> None:
        src = ws / "in.png"
        _make_png(src)
        out = ws / "out.jpg"
        result = asyncio.run(
            media.image_edit(
                str(src), resize="5x5", crop="1,1,4,4", rotate=90, flip="horizontal", format="jpeg", output=str(out)
            )
        )
    ...
    def test_no_output_path(self, ws: Path) -> None:
        src = ws / "in.png"
        _make_png(src)
    ...
    def test_jpeg_default_output(self, ws: Path) -> None:
        src = ws / "in.png"
        _make_png(src)
    ...
    def test_pillow_missing(self, ws: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        import builtins
...
class TestDocumentParse:
    ...
    def test_pptx(self, ws: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_xlsx(self, ws: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_unsupported(self, ws: Path) -> None:
        txt = ws / "notes.txt"
        txt.write_text("hi", encoding="utf-8")
        assert "Unsupported format" in asyncio.run(media.document_parse(str(txt)))

    def test_import_error(self, ws: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_general_error(self, ws: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        def _boom(path: Path, pages: str = "") -> str:
...
class TestParsePdf:
    def test_pages_out_of_range(self, ws: Path) -> None:
        pdf = ws / "doc.pdf"
        _make_pdf(pdf)
        out = media._parse_pdf(pdf, "5,7")
        assert "Extracted 0 chars" in out

    def test_truncated(self, ws: Path) -> None:
        pdf = ws / "doc.pdf"
...
class TestParsePptx:
    def test_parse(self, ws: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        _install_fake_pptx(monkeypatch)
        pptx = ws / "slides.pptx"
        pptx.write_bytes(b"fake")
...
class TestParseXlsx:
    ...
    def test_truncated(self, ws: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestVideoInfo:
    def test_access_denied(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_prefix_collision(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ws_dir = tmp_path / "ws"
    ...
    def test_no_ffmpeg(self, ws: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setitem(sys.modules, "ffmpeg", None)
        vid = ws / "clip.mp4"
        vid.write_bytes(b"x")
        assert "ffmpeg-python not installed" in asyncio.run(media.video_info(str(vid)))

    def test_probe_failure(self, ws: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        mod: Any = types.ModuleType("ffmpeg")
    ...
    def test_success(self, ws: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        mod: Any = types.ModuleType("ffmpeg")
        probe_result = {
            "streams": [
...
class TestVideoThumbnail:
    ...
    def test_success(self, ws: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        def writer(out: str) -> None:
            Image.new("RGB", (4, 4), (0, 128, 0)).save(out, format="JPEG")
    ...
    def test_custom_output(self, ws: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        def writer(out: str) -> None:
            Image.new("RGB", (4, 4), (0, 128, 0)).save(out, format="JPEG")
    ...
    def test_bad_output_image(self, ws: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        def writer(out: str) -> None:
    ...
    def test_extract_failure(self, ws: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        def writer(out: str) -> None:
            raise RuntimeError("boom")
...
class TestVideoTranscribe:
    ...
    def test_no_key(self, ws: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(media, "_OPENAI_API_KEY", "")
        vid = ws / "clip.mp4"
        vid.write_bytes(b"x")
        assert "OPENAI_API_KEY" in asyncio.run(media.video_transcribe(str(vid)))

    def test_no_httpx(self, ws: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(media, "_OPENAI_API_KEY", "sk-test")
    ...
    def test_ffmpeg_failure(self, ws: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(media, "_OPENAI_API_KEY", "sk-test")
    ...
    def test_http_error(self, ws: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(media, "_OPENAI_API_KEY", "sk-test")
...
class TestVideoExtractFrames:
    ...
    def test_no_duration(self, ws: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_output_dir_denied(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch, ws: Path) -> None:
    ...
    def test_output_dir_prefix_denied(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch, ws: Path) -> None:
        _install_fake_ffmpeg_probe(monkeypatch, duration=10)
        vid = ws / "clip.mp4"
        vid.write_bytes(b"x")
    ...
    def test_relative_output_dir(self, ws: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        def writer(out: str) -> None:
            Image.new("RGB", (8, 8), (0, 0, 255)).save(out, format="JPEG")
    ...
    def test_preview_failure(self, ws: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        def writer(out: str) -> None:
    ...
    def test_extraction_failure(self, ws: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        def writer(out: str) -> None:
            raise RuntimeError("boom")
...
class TestImageAnalyze:
    ...
    def test_unsupported_ext(self, ws: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(media, "_OPENAI_API_KEY", "sk-test")
        txt = ws / "notes.txt"
        txt.write_text("hi", encoding="utf-8")
    ...
    def test_mime_variants(self, ws: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(media, "_OPENAI_API_KEY", "sk-test")
    ...
    def test_no_choices(self, ws: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(media, "_OPENAI_API_KEY", "sk-test")
