...
class TestDocumentChunker:
    def test_empty_text(self):
        dc = DocumentChunker()
        chunks = dc.chunk_text("", source="test")
        assert chunks == []

    def test_single_chunk(self):
        dc = DocumentChunker(chunk_size=1000)
        chunks = dc.chunk_text("hello world", source="test")
        assert len(chunks) == 1
        assert chunks[0]["text"] == "hello world"
        assert chunks[0]["source"] == "test"

    def test_multiple_chunks(self):
        dc = DocumentChunker(chunk_size=10, overlap=2)
        text = "abcdefghijklmnopqrstuvwxyz"
        chunks = dc.chunk_text(text)
        assert len(chunks) >= 2
        for c in chunks:
            assert len(c["text"]) <= 10

    def test_break_at_paragraph(self):
        dc = DocumentChunker(chunk_size=50, overlap=0)
    ...
    def test_metadata_in_chunks(self):
        dc = DocumentChunker(chunk_size=50)
        text = "a" * 100
        chunks = dc.chunk_text(text, source="src.txt", metadata={"author": "me"})
        assert len(chunks) >= 2
        for c in chunks:
            assert c["source"] == "src.txt"
            assert c["author"] == "me"

    def test_chunk_file_nonexistent(self, tmp_path: Path):
        dc = DocumentChunker()
        chunks = dc.chunk_file(tmp_path / "nonexistent.txt")
        assert chunks == []

    def test_chunk_file_txt(self, tmp_path: Path):
        f = tmp_path / "test.txt"
        f.write_text("hello world", encoding="utf-8")
        dc = DocumentChunker()
        chunks = dc.chunk_file(f)
        assert len(chunks) == 1
        assert "hello world" in chunks[0]["text"]
        assert chunks[0]["file"] == str(f)

    async def test_retriever_index_and_retrieve(self, tmp_db: Path):
    ...
    async def test_retriever_context(self, tmp_db: Path):
        from raven.core.rag.retriever import Retriever
    ...
    async def test_retriever_index_chunks(self, tmp_db: Path):
        chunks = [
    ...
    async def test_conversation_memory(self, tmp_path: Path):
        from raven.core.rag.memory import ConversationMemory

        db = tmp_path / "mem.db"
        cm = ConversationMemory(db)
        await cm.save_summary("sess1", "hello world", topics=["greeting"])
        summary = await cm.get_summary("sess1")
        assert summary is not None
        assert summary["summary"] == "hello world"
        assert json.loads(summary["topics"]) == ["greeting"]

    async def test_conversation_memory_update(self, tmp_path: Path):
        from raven.core.rag.memory import ConversationMemory

        db = tmp_path / "mem.db"
        cm = ConversationMemory(db)
        await cm.save_summary("sess1", "first", topics=["a"])
        await cm.save_summary("sess1", "second", topics=["b"])
        summary = await cm.get_summary("sess1")
        assert summary is not None
        assert summary["summary"] == "second"

    async def test_conversation_memory_no_results(self, tmp_path: Path):
        from raven.core.rag.memory import ConversationMemory

        db = tmp_path / "mem.db"
        cm = ConversationMemory(db)
        summary = await cm.get_summary("nonexistent")
