...
class TestFilesPlugin:
    ...
    def _allow(self, tmp_path):
        ...
        from raven.plugins.files import plugin as p
        return patch.object(p, 'ALLOWED_ROOTS', [
            str(Path.home()), str(Path.cwd()), "/tmp", str(tmp_path.resolve()),
        ])
...
class TestMemoryPlugin:
    ...
    def test_tool_discovery(self):
        from raven.plugins.memory import plugin as p
        tools = _collect_tools(p)
        for name in ("remember", "recall", "forget", "search_memory", "list_keys", "store_knowledge", "retrieve_knowledge"):
            assert name in tools
