...
async function runReview(api, document) {
    const code = document.getText();
    const filename = document.fileName;
    const result = await api.review(code, filename);
    const diagnostics = [];
    for (const d of result.diagnostics) {
        const range = new vscode.Range(Math.max(0, d.line), Math.max(0, d.column), Math.min(d.endLine, document.lineCount - 1), d.endColumn);
